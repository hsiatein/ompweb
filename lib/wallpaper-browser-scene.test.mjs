...
const { browserEffectGlsl, browserFragmentGlsl, sceneAudioUniforms } = await jiti.import("./wallpaper-glsl.ts");

test('fragment input writes use private per-fragment copies, including helper reads',()=>{
  const out=browserFragmentGlsl('varying vec2 uv;vec2 readUv(){return uv;}void main(){uv.x+=0.1;uv.y=0.2;gl_FragColor=vec4(readUv(),0.0,1.0);}');
  assert.match(out,/varying vec2 uv;\nvec2 we_mutable_uv;/);
  assert.match(out,/main\(\)\{we_mutable_uv=uv;/);
  assert.match(out,/return we_mutable_uv;/);assert.match(out,/we_mutable_uv.x\+=/);
  const readOnly='varying vec2 uv;void main(){if(uv.x==0.0)gl_FragColor=vec4(uv,0.0,1.0);}';
  assert.equal(browserFragmentGlsl(readOnly),readOnly);
});
...
test('numeric conditions preserve integer truncation when sampling loops use floating counters',()=>{
  const out=browserEffectGlsl('uniform float low;uniform float high;uniform float audio[64];void main(){for(int a=int(low);a<=int(high);++a){float v=audio[a];if(v>int(high)){v=0.0;}}}');
  assert.match(out,/a <= float\(int\(high\)\)/);
  assert.match(out,/audio\[int\(a\)\]/);
  assert.match(out,/v > float\(int\(high\)\)/);
});
...
test("scene manifests contain content-addressed PNG assets, never streams, and reject unsupported live behavior", async t => {
  ...
  await fs.writeFile(path.join(root, "project.json"), JSON.stringify({ type: "scene", file: "scene.json", general: { properties: { timevarying: { value: true }, daytime: { value: "9" }, nested: { value: { unsupported: true } } } } }));
  ...
  assert.deepEqual(manifest.userProperties, { timevarying: true, daytime: '9' });
  ...
  const overrideDir = path.join(process.env.PI_CODING_AGENT_DIR, 'web-wallpapers', 'scene-properties');
  const overrideFile = path.join(overrideDir, `${id}.json`);
  await fs.mkdir(overrideDir, { recursive: true });
  await fs.writeFile(overrideFile, JSON.stringify({timevarying:false,daytime:123,nested:'invalid',unknown:true}));
  scene.objects[0].visible={user:'timevarying',value:true};
  scene.objects.push({id:3,image:'models/a.json',size:'2048 1024'});
  data['scene.json']=Buffer.from(JSON.stringify(scene));await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  const hidden = (await compileBrowserScene(id)).manifest;
  assert.equal(hidden.layers.some(l=>l.id===1),false);
  assert.deepEqual(hidden.userProperties,{timevarying:false,daytime:'9'});
  scene.objects[0].visible.script='export function update(value){return value}';
  data['scene.json']=Buffer.from(JSON.stringify(scene));await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  const scriptedOverride=(await compileBrowserScene(id)).manifest.layers.find(l=>l.id===1);
  assert.equal(scriptedOverride.visible,false);assert.equal(scriptedOverride.scripts[0].property,'visible');
  assert.deepEqual(await store.readScenePropertyOverrides('f'.repeat(32)),{});
  await assert.rejects(store.readScenePropertyOverrides('../escape'),/Invalid scene identifier/);
  await fs.writeFile(overrideFile,'[]');await assert.rejects(compileBrowserScene(id),/Invalid scene property overrides/);
  await fs.writeFile(overrideFile,' '.repeat(256*1024+1));await assert.rejects(compileBrowserScene(id),/Invalid metadata/);
  await fs.unlink(overrideFile);
  delete scene.objects[0].visible;scene.objects.pop();
  data['scene.json']=Buffer.from(JSON.stringify(scene));
  await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  ...
  scene.objects[0].colorBlendMode = 21;
  data['scene.json'] = Buffer.from(JSON.stringify(scene)); await fs.writeFile(path.join(root, 'scene.pkg'), pkg(data));
  assert.equal((await compileBrowserScene(id)).manifest.layers[0].colorBlendMode, 21);
  scene.objects[0].colorBlendMode = 31;
  data['scene.json'] = Buffer.from(JSON.stringify(scene)); await fs.writeFile(path.join(root, 'scene.pkg'), pkg(data));
  assert.equal((await compileBrowserScene(id)).manifest.layers[0].colorBlendMode, 31);
  scene.objects[0].colorBlendMode = 99;
  data['scene.json'] = Buffer.from(JSON.stringify(scene)); await fs.writeFile(path.join(root, 'scene.pkg'), pkg(data));
  await assert.rejects(compileBrowserScene(id), /Unsupported scene image blend mode: 99/);
  scene.objects[0].colorBlendMode = 7;
  ...
  const smallFont = data['fonts/test.ttf'];
  data['fonts/test.ttf'] = Buffer.alloc(17 * 1024 * 1024); smallFont.copy(data['fonts/test.ttf']);
  await fs.writeFile(path.join(root, 'scene.pkg'), pkg(data));
  const largeFont = await compileBrowserScene(id);
  assert.equal(largeFont.assets.get(largeFont.manifest.fonts[0].key).length, 17 * 1024 * 1024);
  data['fonts/test.ttf'] = Buffer.alloc(33 * 1024 * 1024); smallFont.copy(data['fonts/test.ttf']);
  await fs.writeFile(path.join(root, 'scene.pkg'), pkg(data));
  await assert.rejects(compileBrowserScene(id), /Scene resource too large/);
  data['fonts/test.ttf'] = smallFont;
  ...
  scene.objects.push({id:20,camera:'default',origin:'1024 512 500',zoom:1.25,path:'scripts/camera.json'});
  data['scripts/camera.json']=Buffer.from('{"paths":[]}');
  data['scene.json']=Buffer.from(JSON.stringify(scene));await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  const camera=(await compileBrowserScene(id)).manifest.layers.find(l=>l.id===20);
  assert.deepEqual(camera.camera,{zoom:1.25});assert.equal(camera.group,true);assert.deepEqual(camera.origin,[1024,512,500]);
  data['scripts/camera.json']=Buffer.from('{"paths":[{"name":"animated"}]}');await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  await assert.rejects(compileBrowserScene(id),/Animated camera paths/);
  scene.objects.pop();
  scene.objects.push({id:21,name:'night',visible:false,image:'models/a.json'});
  scene.objects[0].visible={value:true,script:"export function update(){const video=thisScene.getLayer('night').getVideoTexture();video.play();}"};
  data['scene.json']=Buffer.from(JSON.stringify(scene));await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  assert.equal((await compileBrowserScene(id)).manifest.layers.find(l=>l.id===21).visible,false);
  delete scene.objects[0].visible;scene.objects.pop();
  data['models/compose.json']=Buffer.from('{"material":"materials/compose.json","passthrough":true}');
  data['materials/compose.json']=Buffer.from('{"passes":[{"shader":"composelayer","textures":["_rt_FullFrameBuffer"]}]}');
  scene.objects.push({id:22,image:'models/compose.json',size:'128 128',copybackground:false,colorBlendMode:21});
  data['scene.json']=Buffer.from(JSON.stringify(scene));await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  const composition=(await compileBrowserScene(id)).manifest.layers.find(l=>l.id===22);
  assert.equal(composition.composition,true);assert.equal(composition.texture,'@transparent');
  scene.objects.at(-1).copybackground=true;
  data['scene.json']=Buffer.from(JSON.stringify(scene));await fs.writeFile(path.join(root,'scene.pkg'),pkg(data));
  await assert.rejects(compileBrowserScene(id),/composition/i);
  scene.objects.pop();
